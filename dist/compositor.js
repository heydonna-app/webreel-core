...
export async function compose(cleanVideoPath, timelineData, outputPath, options) {
    ...
    try {
        await compositeFrames(ffmpegPath, cleanVideoPath, timelineData, cursorPng, zoom, tempComposed, crf);
...
async function compositeFrames(ffmpegPath, cleanVideoPath, timeline, cursorPng, zoom, outputPath, crf) {
    ...
    for (let i = 0; i < timeline.frames.length; i++) {
        ...
        const overlayPng = await renderOverlayFrame(frame, width, height, ctx, overlayCache, hudCache);
...
async function renderOverlayFrame(frame, width, height, ctx, cache, hudCache) {
    ...
    const cursorVisible = icx >= -ctx.cursorWidth && icx < width && icy >= -ctx.cursorHeight && icy < height;
    if (cursorVisible) {
